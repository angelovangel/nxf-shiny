...
ui <- page_navbar(
  ...
  # Inject custom CSS here to fix stdout opacity issue and apply flat theme
  tags$head(
...
server <- function(input, output, session) {
  ...
  tmux_sessions <- reactive({
    ...
    if (any(str_detect(tmuxinfo, "no server|error"))) {
      ...
      df$pipeline_time <- sapply(df$session_id, function(x) {
        tail(nxflogs[[x]]$DURATION, 1)
      })
  ...
  output$table <- renderReactable({
    reactable(
      ...
      theme = reactableTheme(
        rowSelectedStyle = list(backgroundColor = "#F3F4F6", boxShadow = "inset 2px 0 0 0 #0047AB;")
      ),
  ...
  observeEvent(input$start, {
    ...
    args1 <- c("new", "-d", "-s", session_id, "-c", instance_path, "-x", "120", "-y", "30", "'bash --login'")
    ...
    withCallingHandlers(
      {
        ...
        p <- processx::run(
          "tmux",
          args = args,
          stdout_callback = function(line, proc) {
            message(line)
          },
