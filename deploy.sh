...
echo -e "${BOLD}${CYAN}===================================================${NC}"
echo -e "${BOLD}${CYAN}       Deploying nxf-shiny on Fresh Server         ${NC}"
echo -e "${BOLD}${CYAN}===================================================${NC}"
...
if [ "$SKIP_SYSTEMD" = false ] && command -v systemctl >/dev/null 2>&1; then
    log_info "Setting up systemd service for nxf-shiny..."
    ...
    # --- Shiny App Service ---
    SHINY_SERVICE_FILE="/etc/systemd/system/nxf-shiny.service"
    cat <<EOF | $SUDO tee "$SHINY_SERVICE_FILE" >/dev/null
[Unit]
Description=NXF Shiny App
...
EOF
    ...
    $SUDO systemctl enable --now nxf-shiny.service

    log_success "Enabled and started nxf-shiny.service."
else
...
if command -v ufw >/dev/null 2>&1; then
    if $SUDO ufw status | grep -qw "active"; then
        ...
        $SUDO ufw allow "${SHINY_PORT}/tcp" comment "NXF Shiny App" >/dev/null 2>&1 || true
...
if [ "$SKIP_SYSTEMD" = false ] && command -v systemctl >/dev/null 2>&1; then
    ...
    # Check Shiny App
    SHINY_STATUS="$(systemctl is-active nxf-shiny.service 2>/dev/null || echo 'inactive')"
    if [ "$SHINY_STATUS" = "active" ]; then
        log_success "nxf-shiny.service is active (running)."
    else
        log_error "nxf-shiny.service failed to start (status: ${SHINY_STATUS}). Recent logs:"
        $SUDO journalctl -u nxf-shiny.service -n 20 --no-pager || true
    fi
...
if command -v systemctl >/dev/null 2>&1 && [ "$SKIP_SYSTEMD" = false ]; then
    echo -e "   nxf-shiny.service: $(systemctl is-active nxf-shiny 2>/dev/null || echo 'inactive')"
fi
echo ""
echo -e "${BOLD}Useful Management Commands:${NC}"
echo -e "   Check status:  sudo systemctl status nxf-shiny"
echo -e "   Restart app:   sudo systemctl restart nxf-shiny"
echo -e "   View app logs: sudo journalctl -u nxf-shiny -f (or tail -f logs/shiny-app.log)"
echo ""
